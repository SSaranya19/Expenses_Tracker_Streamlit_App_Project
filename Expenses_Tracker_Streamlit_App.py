...
questions = {
    "Q1. What is the total amount spent in each category?": """
        Select
        ...
        Order by
	        Total_spent;""",
    "Q2. What is the total amount spent using each payment mode?": """
        Select
    ...
    "Q4. Which are the top 5 most expensive categories in terms of spending?": """
        Select
	        Category, sum(Amount) As Total_spent
        From
	        expense_tb
    ...
    "Q5. How much was spent on transportation using different payment modes?": """
        Select
    ...
    "Q6. Which transactions resulted in cashback?": """
        Select
    ...
    "Q7. What is the total spending in each month of the year?": """
        Select
        ...
        Group by 
            Year_of_Month;""",
    "Q8. Which months have the highest spending in categories like 'Travel,' 'Entertainment,' or 'Gifts'?": """
        Select 
    ...
    "Q9. How much cashback or rewards were earned in each month?": """
        Select
	        strftime('%Y-%m', Date) AS Month,
            SUM(Cashback) AS Total_cashback
        From
        ...
        Order by
	        Month;""",
    "Q10. How has your overall spending changed over time (e.g., increasing, decreasing, remaining stable)?": """
        Select
            strftime('%Y-%m', Date) AS Year_of_Month,
            SUM(Amount) AS Total_Spent
        From
        ...
        Order by
            Year_of_Month;""",
    "Q11. Define High and Low Priority Categories":"""
        Select
	        Category,
            SUM(Amount) AS Total_Spent,
            Case
...
question_options = ["Select a question"] + list(questions.keys())
selected_query = st.sidebar.selectbox("Select a query to display:", options=query_options)
selected_question = st.sidebar.selectbox("Select a question to display:", options=question_options)
...
selected_questions = st.sidebar.multiselect("Select a question to display charts:", options=question_options)
...
# Displaying Question Charts
for question in selected_questions:
    query = questions[question]
    df = pd.read_sql(query, conn)

    if question == "Q1. What is the total amount spent in each category?":
    ...
    if question == "Q2. What is the total amount spent using each payment mode?":
    ...
    if question == "Q4. Which are the top 5 most expensive categories in terms of spending?":
    ...
    if question == "Q5. How much was spent on transportation using different payment modes?":
    ...
    if question == "Q6. Which transactions resulted in cashback?":
    ...
    if question == "Q7. What is the total spending in each month of the year?":
    ...
    if question == "Q8. Which months have the highest spending in categories like 'Travel,' 'Entertainment,' or 'Gifts'?":
    ...
    if question == "Q9. How much cashback or rewards were earned in each month?":
    ...
    if question == "Q10. How has your overall spending changed over time (e.g., increasing, decreasing, remaining stable)?":
    ...
    if question == "Q11. Define High and Low Priority Categories":
